...
noncomputable def twZ (μ φ p : ℝ) : ℝ := μ ^ (2 - p) / (φ * (2 - p))

/-- The `j`-th summand of the integrand `a y * exp(...)`, faithful to the definition of `a`. -/
noncomputable def twG (μ φ p : ℝ) (j : ℕ) (y : ℝ) : ℝ :=
  let α := (2 - p) / (1 - p)
  (1/y) * ((y ^ (- j * α) * (p - 1) ^(α * j)) /
    (φ ^ (j * (1 - α)) * (2 - p) ^ j * Nat.factorial j * Gamma (- j * α)))
  * exp ((1 / φ) * ((μ ^ (1 - p) / (1 - p)) * y - (μ ^ (2 - p) / (2 - p))))

/-- Closed form of `twG j y` for `y > 0`: a constant times `y^(-jα-1) * exp(-(rate)·y)`. -/
lemma tw_pt (μ : ℝ) {φ p : ℝ} (hp₁ : 1 < p) (hφ : 0 < φ) (j : ℕ) {y : ℝ} (hy : 0 < y) :
    twG μ φ p j y
    = (Real.exp (-twZ μ φ p) * (p-1)^(((2-p)/(1-p))*(j:ℝ))
  ...
  have hexp : exp ((1 / φ) * ((μ ^ (1 - p) / (1 - p)) * y - (μ ^ (2 - p) / (2 - p))))
      = exp (-twZ μ φ p) * exp (-(μ ^ (1 - p) / (φ * (p - 1)) * y)) := by
    rw [← exp_add]; congr 1; rw [twZ]; field_simp; ring
  rw [hexp]
  have hypow : (1/y) * (y ^ (-(j:ℝ)*((2-p)/(1-p)))) = y ^ (-(j:ℝ)*((2-p)/(1-p)) - 1) := by
    rw [Real.rpow_sub hy, rpow_one]; ring
  rw [← hypow]; ring

/-- The integrand is the pointwise tsum of the `twG` family. -/
lemma tw_pointwise (μ φ p : ℝ) (y : ℝ) :
    a y φ p * exp ((1 / φ) * ((μ ^ (1 - p) / (1 - p)) * y - (μ ^ (2 - p) / (2 - p))))
      = ∑' j : ℕ, twG μ φ p j y := by
  simp only [a, twG]
  rw [tsum_mul_right, tsum_mul_left]

/-- The zeroth term vanishes identically (`Γ(0) = 0`). -/
lemma twG_zero (μ φ p y : ℝ) : twG μ φ p 0 y = 0 := by
  simp [twG, Gamma_zero]

/-- The summand is nonnegative on `(0, ∞)`. -/
lemma twG_nonneg (μ φ p : ℝ) (hp₁ : 1 < p) (hp₂ : p < 2) (hφ : 0 < φ)
    (j : ℕ) {y : ℝ} (hy : 0 < y) : 0 ≤ twG μ φ p j y := by
  cases j with
  | zero => rw [twG_zero]
  | succ n =>
    set j := n + 1
...
/-- Each `twG j` is integrable on `(0, ∞)`. -/
lemma tw_integrable_on {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ)
    (j : ℕ) : IntegrableOn (fun y => twG μ φ p j y) (Set.Ioi 0) := by
  rcases Nat.eq_zero_or_pos j with hj0 | hjpos0
  · subst hj0; simp only [twG_zero]; exact integrableOn_zero
  · have hjpos : 0 < (j:ℝ) := by exact_mod_cast hjpos0
    ...
    have hconst : IntegrableOn
        (fun y => (Real.exp (-twZ μ φ p) * (p-1)^(((2-p)/(1-p))*(j:ℝ))
          / (φ^((j:ℝ)*(1-(2-p)/(1-p))) * (2-p)^j * (Nat.factorial j)
            * Gamma (-(j:ℝ)*((2-p)/(1-p)))))
          * (y ^ (-(j:ℝ)*((2-p)/(1-p)) - 1)
...
lemma tw_integral_term {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ)
    {j : ℕ} (hj : 1 ≤ j) :
    ∫ y in Set.Ioi (0:ℝ), twG μ φ p j y
      = exp (-twZ μ φ p) * (twZ μ φ p) ^ j / (Nat.factorial j) := by
  have h1p : (1 - p) ≠ 0 := by linarith
...
lemma tw_integral_zero (μ φ p : ℝ) :
    ∫ y in Set.Ioi (0:ℝ), twG μ φ p 0 y = 0 := by
  simp [twG_zero]

/-- Summability of the integral norms (needed to swap `∫` and `∑'`). -/
lemma tw_summable_norm (μ φ p : ℝ) (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
    Summable (fun j : ℕ => ∫ y in Set.Ioi (0:ℝ), ‖twG μ φ p j y‖) := by
  have hsum2 : Summable (fun j : ℕ =>
      exp (-twZ μ φ p) * (twZ μ φ p) ^ j / (Nat.factorial j)) := by
    have := (Real.summable_pow_div_factorial (twZ μ φ p)).mul_left (Real.exp (-twZ μ φ p))
    simpa [mul_div_assoc] using this
  have hnorm_eq (j) : ∫ y in Set.Ioi 0, ‖twG μ φ p j y‖
      = ∫ y in Set.Ioi 0, twG μ φ p j y := by
    apply setIntegral_congr_fun measurableSet_Ioi
    intro y hy; simp only [Set.mem_Ioi] at hy
    exact norm_of_nonneg (twG_nonneg μ φ p hp₁ hp₂ hφ j hy)
  apply Summable.of_nonneg_of_le _ _ hsum2
  · intro j; rw [hnorm_eq]
    exact setIntegral_nonneg measurableSet_Ioi
      fun y => twG_nonneg μ φ p hp₁ hp₂ hφ j
  · intro j
    rw [hnorm_eq]
...
lemma tw_tsum (μ φ p : ℝ) :
    ∑' j : ℕ, (if j = 0 then (0:ℝ)
      else exp (-twZ μ φ p) * (twZ μ φ p) ^ j / (Nat.factorial j))
      = 1 - exp (-twZ μ φ p) := by
  set z := twZ μ φ p
...
/-- `ENNReal`-valued Tweedie probability density function. -/
...
def tweedieMeasure (μ : ℝ) {φ p : ℝ} (hφ : 0 ≤ φ)
    ...
    (tweedieProbZero μ φ p) • (Measure.dirac 0)
...
lemma tweedieMeasure_prob (μ : ℝ) (hμ : 0 < μ) {φ p : ℝ} (hφ : 0 < φ)
  ...
  have : ∫⁻ (a : ℝ), tweediePDF' μ hp₁ (by linarith) (show 0 ≤ φ by linarith) a =
    ...
    have : (1 : NNReal) - ⟨rexp (-μ ^ (2 - p) / (φ * (2 - p))), exp_nonneg _⟩
      = ⟨1 - tweedieProbZero μ φ p, by
        generalize tweedieProbZero μ φ p = α at *
    ...
    have : (1 : ENNReal) - ENNReal.ofNNReal ⟨rexp (-μ ^ (2 - p) / (φ * (2 - p))), exp_nonneg _⟩
      = ENNReal.ofNNReal ⟨1 - tweedieProbZero μ φ p,
      sub_nonneg_of_le (by simp;tauto)⟩ := by rw [← this]; simp
    unfold tweedieProbZero
    rw [this]
    simp only [tweedieProbZero]
...
/-- The Tweedie measure as a probability measure. -/
...
/-
Closed form of `y * twG j y` for `y > 0`: the same constant as in `tw_pt`, but with
...
/-
Each `y * twG j` is integrable on `(0, ∞)`.
-/
lemma tw_yG_integrable_on {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ)
    (j : ℕ) : IntegrableOn (fun y => y * twG μ φ p j y) (Set.Ioi 0) := by
  rcases Nat.eq_zero_or_pos j with hj0 | hjpos0
  · simp [hj0, twG_zero]
  · have h_integrable : IntegrableOn (fun y => y ^ (-(j:ℝ)*((2-p)/(1-p))) * Real.exp (-(μ ^ (1 - p)
        / (φ * (p - 1)) * y))) (Set.Ioi 0) := by
      have h_integrable : ∀ {s b : ℝ}, -1 < s → 0 < b →
        IntegrableOn (fun y => y ^ s * Real.exp (-b * y)) (Set.Ioi 0) := by
        intro s b hs hb
        convert (integrableOn_rpow_mul_exp_neg_mul_rpow (show -1 < s by linarith)
            (show 1 ≤ (1 : ℝ) by norm_num) hb) using 1; norm_num
        · rfl
        · simp
      ...
      · exact - (j : ℝ) * ((2 - p) / (1 - p))
      · exact μ ^ (1 - p) / (φ * (p - 1))
      · nlinarith [show (j : ℝ) ≥ 1 by
            norm_cast, mul_div_cancel₀ (2 - p) (by linarith : (1 - p) ≠ 0)]
      · exact div_pos (Real.rpow_pos_of_pos hμ _) (mul_pos hφ (by linarith))
      · norm_num
    refine h_integrable.const_mul ?_ |> fun h => h.congr ?_
    · exact (Real.exp (-twZ μ φ p) * (p - 1) ^ (((2 - p) / (1 - p)) * j) / (φ ^ ((j : ℝ) *
            (1 - (2 - p) / (1 - p))) * (2 - p) ^ j * (Nat.factorial j) * Real.Gamma (- (j : ℝ)
            * ((2 - p) / (1 - p)))))
    filter_upwards [MeasureTheory.ae_restrict_mem measurableSet_Ioi] with y hy
        using by rw [tw_yG_pt μ hp₁ hφ j hy]

/-
The per-term mean integral: `∫ y * twG j = K * (j · exp(-z) z^j / j!)`,
...
lemma tw_mean_term {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) (j : ℕ) :
    ∫ y in Set.Ioi (0:ℝ), y * twG μ φ p j y
      = (φ * (2-p) / μ^(1-p))
        * ((j:ℝ) * Real.exp (-twZ μ φ p) * (twZ μ φ p) ^ j / (Nat.factorial j)) := by
  by_cases hj : j = 0
  · simp_all [twG]
  simp_all only [twG, one_div, neg_mul]
  have h_integral : ∫ y in Set.Ioi (0:ℝ), y * twG μ φ p j y
    = ∫ y in Set.Ioi (0:ℝ), (Real.exp (-twZ μ φ p) * (p-1)^(((2-p)/(1-p))*(j:ℝ))
    / (φ^((j:ℝ)*(1-(2-p)/(1-p))) * (2-p)^j * (Nat.factorial j) * Real.Gamma (-(j:ℝ)*((2-p)/(1-p)))))
    * (y ^ (-(j:ℝ)*((2-p)/(1-p))) * Real.exp (-(μ ^ (1 - p) / (φ * (p - 1)) * y))) := by
  ...
  have h_gamma : ∫ y in Set.Ioi (0:ℝ), y ^ (-(j:ℝ)*((2-p)/(1-p))) * Real.exp (-(μ ^ (1 - p)
    / (φ * (p - 1)) * y)) = (1 / (μ ^ (1 - p) / (φ * (p - 1)))) ^ (-(j:ℝ)*((2-p)/(1-p)) + 1)
    * Real.Gamma (-(j:ℝ)*((2-p)/(1-p)) + 1) := by
    ...
    · nlinarith [show (j : ℝ) ≥ 1 by exact Nat.one_le_cast.mpr (Nat.pos_of_ne_zero hj),
            mul_div_cancel₀ (2 - p) (by linarith : (1 - p) ≠ 0)]
    · exact div_pos (Real.rpow_pos_of_pos hμ _) (mul_pos hφ (by linarith))
  convert h_integral using 1
  · unfold twG; congr; ext
    ring_nf
      ...
      · field_simp
        ...
        · unfold twZ; ring_nf
          rw [show (-1 + p) = (p - 1) by ring,
            show (p * φ * (μ ^ (1 - p)) ⁻¹ - φ * (μ ^ (1 - p)) ⁻¹)
                = (p - 1) * φ * (μ ^ (1 - p)) ⁻¹ by ring]
          rw [Real.mul_rpow (by nlinarith) (by positivity),
            Real.mul_rpow (by nlinarith) (by positivity)]; ring_nf
          rw [show (- (p * j * (1 - p) ⁻¹) + j * (1 - p) ⁻¹ * 2 : ℝ)
            = - (p * j * (1 - p) ⁻¹ - j * (1 - p) ⁻¹ * 2) by ring, Real.rpow_neg (by linarith)]
          ring_nf
          rw [show (-1 + p) = (p - 1) by ring, Real.rpow_def_of_pos (by linarith),
            Real.rpow_def_of_pos (by positivity), Real.rpow_def_of_pos (by positivity)]; ring_nf
          rw [Real.log_inv, Real.log_rpow (by positivity)]; ring_nf
          rw [Real.rpow_def_of_pos (by positivity)]; ring_nf
          rw [Real.rpow_def_of_pos (by positivity)]; ring_nf
          rw [show (- (p * φ) + φ * 2) = φ * (2 - p) by ring, mul_inv]
          norm_num [Real.exp_add, Real.exp_neg, Real.exp_nat_mul, Real.exp_log, hμ, hφ, hp₁, hp₂]
          ring_nf
          norm_num [← Real.exp_nat_mul, ← Real.exp_neg, ← Real.exp_add, ← Real.exp_sub,
            hμ.ne', hφ.ne', show (2 - p) ≠ 0 by linarith]; ring_nf
          norm_num [mul_assoc, ← Real.exp_add]; ring_nf
          grind +revert
        · exact mul_ne_zero (mul_ne_zero (by linarith) (pow_ne_zero _ (by linarith))) (ne_of_gt
                (Real.Gamma_pos_of_pos (
                    by nlinarith [
                        show (j : ℝ) ≥ 1 by exact Nat.one_le_cast.mpr (Nat.pos_of_ne_zero hj),
                        mul_div_cancel₀ ((2 - p) * j) (by linarith : (1 - p) ≠ 0)])))
      · exact one_div_pos.mpr (div_pos (Real.rpow_pos_of_pos hμ _) (mul_pos hφ (by linarith)))
    · exact mul_ne_zero (neg_ne_zero.mpr (Nat.cast_ne_zero.mpr hj)) (div_ne_zero (by linarith)
        (by linarith))

/-- Summability of `fun j => (j:ℝ) * z^j / j!` for any real `z`. -/
lemma tw_summable_n_pow (z : ℝ) :
    Summable (fun j : ℕ => (j:ℝ) * z ^ j / (Nat.factorial j)) := by
  refine summable_of_ratio_norm_eventually_le ?_ ?_ (r := 2 / 3)
  · norm_num
  · norm_num [Nat.factorial_succ, abs_div, abs_mul]
    refine ⟨⌈3 * |z|⌉₊ + 1, fun n hn => ?_⟩; rw [mul_div_mul_left _ _ (by positivity)]; ring_nf
    nlinarith [show (n : ℝ) ≥ ⌈3 * |z|⌉₊ + 1 by exact_mod_cast hn, Nat.le_ceil (3 * |z|),
        show (0 : ℝ) ≤ |z| ^ n * (n.factorial : ℝ) ⁻¹ by positivity]

/-- The "Poisson mean" identity: `∑' j, j z^j / j! = z · exp z`. -/
lemma tw_tsum_n_pow (z : ℝ) :
    ∑' j : ℕ, (j:ℝ) * z ^ j / (Nat.factorial j) = z * Real.exp z := by
  convert (Summable.tsum_eq_zero_add
    (show Summable fun j : ℕ => ((j : ℝ) * z ^ j / (j.factorial : ℝ)) from ?_)) using 1
  · norm_num [Nat.factorial_succ, pow_succ', mul_assoc, mul_comm, mul_left_comm, div_eq_mul_inv,
        tsum_mul_left]
    field_simp
    rw [Real.exp_eq_exp_ℝ, NormedSpace.exp_eq_tsum_div]
...
lemma tw_mean_summable_norm (μ φ p : ℝ) (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
    Summable (fun j : ℕ => ∫ y in Set.Ioi (0:ℝ), ‖y * twG μ φ p j y‖) := by
  convert Summable.mul_left (φ * (2 - p) / μ ^ (1 - p) * Real.exp (-twZ μ φ p))
    (tw_summable_n_pow (twZ μ φ p)) using 2
  · rfl
  rw [MeasureTheory.setIntegral_congr_fun measurableSet_Ioi fun y hy =>
    Real.norm_of_nonneg (mul_nonneg hy.out.le (twG_nonneg μ φ p hp₁ hp₂ hφ _ hy))];
    rw [tw_mean_term hp₁ hp₂ hμ hφ]
  ring
...
lemma tw_mean_tsum {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
    ∑' j : ℕ, (φ * (2-p) / μ^(1-p))
        * ((j:ℝ) * Real.exp (-twZ μ φ p) * (twZ μ φ p) ^ j / (Nat.factorial j)) = μ := by
  -- Let `z = twZ μ φ p` and `K = φ*(2-p)/μ^(1-p)`.
  -- Each summand is `K * ((j:ℝ) * exp(-z) * z^j / j!)`.
  set z := twZ μ φ p
  set K := φ * (2 - p) / μ ^ (1 - p) with hK
  -- So the sum equals `K * exp(-z) * (z * exp z) = K * z * (exp (-z) * exp z) = K * z`
  -- (since `exp (-z) * exp z = 1`, via `Real.exp_neg` and `inv_mul_cancel` or `← Real.exp_add`).
  have hsum : ∑' j : ℕ, (j : ℝ) * Real.exp (-z) * z ^ j / (Nat.factorial j)
    = z * Real.exp z * Real.exp (-z) := by
    have := @tw_tsum_n_pow z
    simp_all +decide only [div_eq_mul_inv, mul_comm, mul_left_comm, mul_assoc]
  ...
  · simp +zetaDelta only at *
    unfold twZ; norm_num [mul_assoc, ← Real.exp_add]; ring_nf
    rw [show (2 - p) = (1 - p) + 1 by ring, Real.rpow_add hμ, Real.rpow_one]; ring_nf
    nlinarith [mul_inv_cancel_left₀ (show φ * 2 - φ * p ≠ 0 by nlinarith) μ, mul_inv_cancel₀
        (show μ ^ (1 - p) ≠ 0 by positivity)]
...
lemma tweedie_mean_value {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
  ...
  -- First, show `tweediePDF μ φ p y * y = Set.indicator (Set.Ioi 0)
  -- (fun y => (a y φ p * Real.exp (...)) * y) y`.
  have h_indicator : (∫ y, tweediePDF μ φ p y * y)
    = (∫ y in Set.Ioi (0:ℝ), (a y φ p * Real.exp ((1 / φ) * ((μ ^ (1 - p) / (1 - p))
        * y - (μ ^ (2 - p) / (2 - p)))) * y)) := by
      rw [← MeasureTheory.integral_indicator] <;> norm_num [Set.indicator, tweediePDF]
  rw [h_indicator]
  -- On `Ioi 0`, `(a y φ p * Real.exp (...)) * y = y * (a y φ p * Real.exp (...))
  -- = y * ∑' j, twG μ φ p j y = ∑' j, y * twG μ φ p j y`
  -- (use `tw_pointwise` then `tsum_mul_left`). Apply `setIntegral_congr_fun measurableSet_Ioi`.
  have h_indicator : (∫ y in Set.Ioi (0:ℝ), (a y φ p * Real.exp ((1 / φ) * ((μ ^ (1 - p) / (1 - p))
    * y - (μ ^ (2 - p) / (2 - p)))) * y))
    = (∫ y in Set.Ioi (0:ℝ), ∑' j : ℕ, y * twG μ φ p j y) := by
...
theorem tweedieMeasure_expectation {μ φ p : ℝ} (hμ : 0 < μ) (hφ : 0 < φ)
  ...
  -- Show that `volume.withDensity (tweediePDF' μ hp₁ (by linarith) _)`
  -- equals `volume.withDensity (fun y => ↑(f y))`.
  have h_withDensity : volume.withDensity (tweediePDF' μ hp₁ (by linarith) hφ.le)
    = volume.withDensity (fun y => f y) := by
  ...
  have h_integrable : Integrable (fun y => y) (volume.withDensity (fun y => f y)) := by
    have h_integrable : Integrable (fun y => f y • y) volume := by
      ...
      ext y; exact mul_eq_mul_right_iff.mpr (Or.inl <| Real.coe_toNNReal _ <|
        tweediePDF_nonneg (by linarith) hp₁ (by linarith))
    ...
    · have h_integrable : Integrable (fun y => tweediePDF μ φ p y) volume := by
        ...
        rw [MeasureTheory.integral_undef h_integrable] at this
        linarith [Real.exp_pos (-μ ^ (2 - p) / (φ * (2 - p))),
            Real.exp_lt_one_iff.mpr (show -μ ^ (2 - p) / (φ * (2 - p)) < 0 by
                exact div_neg_of_neg_of_pos (neg_neg_of_pos (Real.rpow_pos_of_pos hμ _))
                    (mul_pos hφ (by linarith)))]
      exact h_integrable.1.aemeasurable.real_toNNReal
  convert congr_arg (fun x : ℝ => x + 0) (tweedie_mean_value hp₁ hp₂ hμ hφ) using 1
  · rw [tweedieMeasure, MeasureTheory.integral_add_measure]
    · norm_num [h_withDensity, h_integrable]
      · convert integral_withDensity_eq_integral_smul₀ _ (fun y => y) using 1
        · simp +zetaDelta only
          congr! 1
          ext y; simp +decide [Real.toNNReal_of_nonneg (tweediePDF_nonneg
            (by linarith : 0 ≤ φ) hp₁ (by linarith))]
          rfl
        · have h_integrable : Integrable (fun y => tweediePDF μ φ p y) volume := by
            exact (by
...
lemma tw_y2G_pt (μ : ℝ) {φ p : ℝ} (hp₁ : 1 < p) (hφ : 0 < φ) (j : ℕ) {y : ℝ} (hy : 0 < y) :
  ...
  convert congr_arg (fun x : ℝ => y * x) (tw_yG_pt μ hp₁ hφ j hy) using 1
  · ring
...
lemma tw_y2G_integrable_on {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ)
    (j : ℕ) : IntegrableOn (fun y => y ^ 2 * twG μ φ p j y) (Set.Ioi 0) := by
  by_cases hj : j = 0
  · unfold twG; aesop
  · have h_integrable : IntegrableOn (fun y => y ^ (-(j:ℝ)*((2-p)/(1-p)) + 1)
        * Real.exp (-(μ ^ (1 - p) / (φ * (p - 1)) * y))) (Set.Ioi 0) := by
      have h_integrable : ∀ {s b : ℝ}, -1 < s → 0 < b → IntegrableOn (fun y => y ^ s
        * Real.exp (-b * y)) (Set.Ioi 0) := by
        intro s b hs hb
        convert (integrableOn_rpow_mul_exp_neg_mul_rpow (show -1 < s by linarith)
            (show 1 ≤ (1 : ℝ) by norm_num) hb) using 1; norm_num
        rfl
      ...
      · exact div_pos (Real.rpow_pos_of_pos hμ _) (mul_pos hφ (by linarith))
    refine h_integrable.const_mul ?_ |> fun h => h.congr ?_
    · exact Real.exp (-twZ μ φ p) * (p - 1) ^ (((2 - p) / (1 - p)) * j)
        / (φ ^ ((j : ℝ) * (1 - (2 - p) / (1 - p))) * (2 - p) ^ j * (j.factorial : ℝ)
        * Real.Gamma (- (j : ℝ) * ((2 - p) / (1 - p))))
    filter_upwards [ MeasureTheory.ae_restrict_mem measurableSet_Ioi ] with y hy using by
        rw [ tw_y2G_pt μ hp₁ hφ j hy ]
...
lemma tw_2nd_moment_recurrence {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ)
    (j : ℕ) :
    ∫ y in Set.Ioi (0:ℝ), y ^ 2 * twG μ φ p j y
      = ((-(j:ℝ)*((2-p)/(1-p)) + 1) * (φ * (p-1) / μ^(1-p)))
        * ∫ y in Set.Ioi (0:ℝ), y * twG μ φ p j y := by
  by_cases hj : j = 0
  · unfold twG; aesop
  · have h_integral : ∫ y in Set.Ioi (0:ℝ), y ^ 2 * twG μ φ p j y
        = (∫ y in Set.Ioi (0:ℝ), y ^ (-(j:ℝ)*((2-p)/(1-p)) + 1)
        * Real.exp (-(μ ^ (1 - p) / (φ * (p - 1)) * y))) * (Real.exp (-twZ μ φ p)
        * (p-1)^(((2-p)/(1-p))*(j:ℝ)) / (φ^((j:ℝ)*(1-(2-p)/(1-p))) * (2-p)^j * (Nat.factorial j)
        * Real.Gamma (-(j:ℝ)*((2-p)/(1-p))))) := by
      rw [ ← MeasureTheory.integral_mul_const ]
      refine MeasureTheory.setIntegral_congr_fun measurableSet_Ioi fun y hy => ?_
      rw [ tw_y2G_pt μ hp₁ hφ j hy ]; ring
    have h_integral : ∫ y in Set.Ioi (0:ℝ), y * twG μ φ p j y
        = (∫ y in Set.Ioi (0:ℝ), y ^ (-(j:ℝ)*((2-p)/(1-p))) * Real.exp (-(μ ^ (1 - p)
        / (φ * (p - 1)) * y))) * (Real.exp (-twZ μ φ p) * (p-1)^(((2-p)/(1-p))*(j:ℝ)) / (φ^((j:ℝ)
        * (1-(2-p)/(1-p))) * (2-p)^j * (Nat.factorial j) * Real.Gamma (-(j:ℝ)*((2-p)/(1-p))))) := by
      rw [ ← MeasureTheory.integral_mul_const ]
      refine MeasureTheory.setIntegral_congr_fun measurableSet_Ioi fun y hy => ?_
      rw [ tw_yG_pt μ hp₁ hφ j hy ]; ring
    rw [ ‹∫ y in Set.Ioi 0, y ^ 2 * twG μ φ p j y = _›, h_integral ]
    have h_integral : ∫ y in Set.Ioi (0:ℝ), y ^ (-(j:ℝ)*((2-p)/(1-p)) + 1) * Real.exp (-(μ ^ (1 - p)
        / (φ * (p - 1)) * y)) = (1 / (μ ^ (1 - p) / (φ * (p - 1)))) ^ (-(j:ℝ)*((2-p)/(1-p)) + 2)
        * Real.Gamma (-(j:ℝ)*((2-p)/(1-p)) + 2) := by
      convert integral_rpow_mul_exp_neg_mul_Ioi _ _ using 1
      · exact MeasureTheory.setIntegral_congr_fun measurableSet_Ioi fun x hx => by ring_nf
      · nlinarith [ show (j : ℝ) ≥ 1 by exact Nat.one_le_cast.mpr (Nat.pos_of_ne_zero hj),
            mul_div_cancel₀ (2 - p) (by linarith : (1 - p) ≠ 0) ]
      · exact div_pos (Real.rpow_pos_of_pos hμ _) (mul_pos hφ (by linarith))
    rw [ h_integral, show (∫ y in Set.Ioi 0, y ^ (- (j : ℝ) * ((2 - p) / (1 - p)))
        * Real.exp (- (μ ^ (1 - p) / (φ * (p - 1)) * y))) = (1 / (μ ^ (1 - p)
        / (φ * (p - 1)))) ^ (- (j : ℝ) * ((2 - p) / (1 - p)) + 1) * Real.Gamma (- (j : ℝ) * ((2 - p)
        / (1 - p)) + 1) from ?_ ]
    · rw [ show (-j * ((2 - p) / (1 - p)) + 2 : ℝ)
        = (-j * ((2 - p) / (1 - p)) + 1) + 1 by ring, Real.rpow_add_one ] <;> norm_num
      · rw [ show (- (j * ((2 - p) / (1 - p))) + 1 + 1 : ℝ)
            = (- (j * ((2 - p) / (1 - p))) + 1) + 1 by ring,
            Real.Gamma_add_one (by nlinarith [
                show (j : ℝ) ≥ 1 by exact Nat.one_le_cast.mpr (Nat.pos_of_ne_zero hj),
                    mul_div_cancel₀ (2 - p) (by linarith : (1 - p) ≠ 0)
                ]
            )
...
lemma tw_2nd_moment_term {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) (j : ℕ) :
    ∫ y in Set.Ioi (0:ℝ), y ^ 2 * twG μ φ p j y
      = (φ * (2-p) / μ^(1-p)) * (φ * (p-1) / μ^(1-p))
        * (((j:ℝ) + (2-p)/(p-1) * (j:ℝ)^2) * Real.exp (-twZ μ φ p)
            * (twZ μ φ p) ^ j / (Nat.factorial j)) := by
...
lemma tw_summable_n2_pow (z : ℝ) :
  ...
  · -- For large enough $n$, the term $(n+1) * |z| / n^2$ will be less than $2/3$.
    have h_bound : ∃ N : ℕ, ∀ n ≥ N, (n + 1) * |z| / n^2 ≤ 2 / 3 := by
      exact ⟨⌈3 * |z|⌉₊ + 1, fun n hn => by rw [
            div_le_iff₀ ] <;> nlinarith [ Nat.le_ceil (3 * |z|),
            show (n : ℝ) ≥ ⌈3 * |z|⌉₊ + 1 by exact_mod_cast hn, abs_nonneg z ]⟩
    obtain ⟨N, hN⟩ := h_bound
    filter_upwards [ Filter.eventually_ge_atTop N, Filter.eventually_gt_atTop 0 ] with n hn hn'
    specialize hN n hn
    simp_all +decide only [Nat.cast_add, Nat.cast_one, Nat.factorial_succ, Nat.cast_mul, norm_div,
      norm_mul, norm_pow, norm_eq_abs, sq_abs, RCLike.norm_natCast]
    convert mul_le_mul_of_nonneg_right hN
        (show 0 ≤ (n ^ 2 * |z| ^ n / n.factorial : ℝ) by positivity) using 1; norm_cast; ring_nf
...
lemma tw_tsum_n2_pow (z : ℝ) :
  ...
  have h1 : ∑' j : ℕ, (j * (j - 1) : ℝ) * z ^ j / j.factorial = z^2 * Real.exp z := by
    ...
    have h_split : ∑' j : ℕ, (j * (j - 1) : ℝ) * z^j / j.factorial
        = ∑' j : ℕ, if j ≥ 2 then (j * (j - 1) : ℝ) * z^j / j.factorial else 0 := by
      exact tsum_congr fun n => by rcases n with (_ | _ | n) <;> norm_num
    -- For $j \geq 2$, we can simplify the term
    -- $(j * (j - 1) : ℝ) * z^j / j.factorial$ to $z^2 * z^{j-2} / (j-2)!$.
    have h_simplify : ∀ j : ℕ, j ≥ 2 → (j * (j - 1) : ℝ) * z^j / j.factorial
        = z^2 * z^(j-2) / (j-2).factorial := by
    ...
    have h_sum_simplified : ∑' j : ℕ, (if j ≥ 2 then (j * (j - 1) : ℝ) * z^j / j.factorial else 0)
        = ∑' j : ℕ, z^2 * z^j / j.factorial := by
      rw [ ← tsum_eq_tsum_of_ne_zero_bij ]
      · use fun x => x.val - 2
      · exact fun x y h => by
            rcases x with ⟨_ | _ | x, hx⟩ <;> rcases y with ⟨_ | _ | y, hy⟩ <;> cases h <;> trivial
      · intro x hx; use ⟨x + 2, by aesop⟩; aesop
      · aesop
    simp_all +decide only [ge_iff_le, exp_eq_exp_ℝ, NormedSpace.exp_eq_tsum_div]
    simp +decide only [mul_div_assoc]
    exact _root_.tsum_mul_left
...
-/
lemma tw_2nd_moment_summable_norm (μ φ p : ℝ)
    (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
    Summable (fun j : ℕ => ∫ y in Set.Ioi (0:ℝ), ‖y ^ 2 * twG μ φ p j y‖) := by
  have h_summable : Summable (fun j : ℕ => ∫ y in Set.Ioi (0:ℝ), y ^ 2 * twG μ φ p j y) := by
    simp_all +decide only [tw_2nd_moment_term]
    refine Summable.mul_left _ ?_
    convert Summable.add (Summable.mul_left (Real.exp (-twZ μ φ p))
        (tw_summable_n_pow (twZ μ φ p))) (Summable.mul_left (Real.exp (-twZ μ φ p)
        * (2 - p) / (p - 1)) (tw_summable_n2_pow (twZ μ φ p))) using 2; ring
  convert h_summable using 1
  generalize_proofs at *
  exact funext fun j => MeasureTheory.setIntegral_congr_fun measurableSet_Ioi fun y hy => by
    rw [ Real.norm_of_nonneg (mul_nonneg (sq_nonneg _) (twG_nonneg μ φ p hp₁ hp₂ hφ j hy)) ]
...
lemma tw_2nd_moment_tsum {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
    ∑' j : ℕ, (φ * (2-p) / μ^(1-p)) * (φ * (p-1) / μ^(1-p))
        * (((j:ℝ) + (2-p)/(p-1) * (j:ℝ)^2) * Real.exp (-twZ μ φ p)
            * (twZ μ φ p) ^ j / (Nat.factorial j))
      = μ^2 + φ * μ^p := by
  -- Factor out common terms and apply the series summations.
  have h_series : ∑' j : ℕ, (j + (2 - p) / (p - 1) * j^2) * Real.exp (-twZ μ φ p)
        * (twZ μ φ p) ^ j / j.factorial =
        Real.exp (-twZ μ φ p) * (twZ μ φ p * Real.exp (twZ μ φ p) + (2 - p) / (p - 1)
        * ((twZ μ φ p)^2 + twZ μ φ p) * Real.exp (twZ μ φ p)) := by
      have h_series : ∑' j : ℕ, (j : ℝ) * twZ μ φ p ^ j / j.factorial = twZ μ φ p
        * Real.exp (twZ μ φ p) ∧ ∑' j : ℕ, (j : ℝ) ^ 2 * twZ μ φ p ^ j / j.factorial
        = (twZ μ φ p ^ 2 + twZ μ φ p) * Real.exp (twZ μ φ p) := by
        exact ⟨by simpa using tw_tsum_n_pow (twZ μ φ p),
               by simpa using tw_tsum_n2_pow (twZ μ φ p)⟩
      convert congr_arg₂ (· + ·) (congr_arg (fun x : ℝ => x * Real.exp (-twZ μ φ p)) h_series.1)
        (congr_arg (fun x : ℝ => x * (2 - p) * Real.exp (-twZ μ φ p) / (p - 1)) h_series.2) using 1
      · norm_num [ add_mul, mul_assoc, mul_div_assoc, tsum_mul_left, tsum_mul_right ]
        ...
        · rw [ ← Summable.tsum_add ]
          ...
          · convert Summable.mul_right ((2 - p) * (Real.exp (-twZ μ φ p) / (p - 1)))
                (tw_summable_n2_pow (twZ μ φ p)) using 2
            · rfl
            · field_simp
        all_goals infer_instance
      · ring
  convert congr_arg (fun x : ℝ => (φ * (2 - p) / μ ^ (1 - p)) * (φ * (p - 1) / μ ^ (1 - p)) * x)
    h_series using 1
  · exact _root_.tsum_mul_left
  · unfold twZ; norm_num [ Real.rpow_sub hμ ]; ring_nf
...
lemma tweedie_2nd_moment_value {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
  ...
  -- Apply the decomposition of the integral into the sum of integrals over the components
  -- and use the fact that the integral of `y^2` with respect to the point mass part is 0.
  have h_decomp : ∫ y, tweediePDF μ φ p y * y ^ 2
    = ∫ y in Set.Ioi (0:ℝ), ∑' j : ℕ, y ^ 2 * twG μ φ p j y := by
    rw [ ← MeasureTheory.integral_indicator ] <;> norm_num [ Set.indicator ]
    congr with x
    by_cases hx : 0 < x
    · simp +decide only [tweediePDF, one_div, mem_setOf_eq, hx, indicator_of_mem, ↓reduceIte]
      convert congr_arg (fun y : ℝ => x ^ 2 * y) (tw_pointwise μ φ p x) using 1
      · ring_nf
      · exact _root_.tsum_mul_left
    · simp +decide [ hx, tweediePDF ]
  rw [ h_decomp, ← tw_2nd_moment_tsum hp₁ hp₂ hμ hφ,
...
`tweediePDF μ φ p y * y²` is integrable.
-/
lemma tweedie_2nd_moment_integrable {μ φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2)
    (hμ : 0 < μ) (hφ : 0 < φ) :
...
theorem tweedieMeasure_2nd_moment {μ φ p : ℝ} (hμ : 0 < μ) (hφ : 0 < φ)
  ...
  · generalize_proofs at *
    · unfold tweedieMeasure; norm_num [ h_withDensity ]
      rw [ MeasureTheory.integral_add_measure ] <;> norm_num [ tweedieProbZero ]
      · convert integral_withDensity_eq_integral_smul₀ _ (fun y => y ^ 2) using 1
        · congr! 1
          generalize_proofs at *
          ext; simp [f]; ring_nf
          exact mul_eq_mul_right_iff.mpr (Or.inl <| by
            rw [ Real.toNNReal_of_nonneg <| tweediePDF_nonneg (by linarith) hp₁ (by linarith) ]
            norm_cast)
        · have h_integrable : MeasureTheory.Integrable (fun y => tweediePDF μ φ p y) volume := by
            exact (by
              ...
              linarith [ Real.exp_pos (-μ ^ (2 - p) / (φ * (2 - p))), Real.exp_lt_one_iff.mpr
                (show -μ ^ (2 - p) / (φ * (2 - p)) < 0 by
                exact div_neg_of_neg_of_pos (neg_neg_of_pos (Real.rpow_pos_of_pos hμ _))
                    (mul_pos hφ (by linarith))) ])
          generalize_proofs at *
          exact h_integrable.1.aemeasurable.real_toNNReal
      ...
      · have h_integrable : Integrable (fun y => f y • y ^ 2) volume := by
          ...
          generalize_proofs at *
          ext y
          exact mul_eq_mul_right_iff.mpr (Or.inl <| Real.coe_toNNReal _ <|
            tweediePDF_nonneg (by linarith) hp₁ (by linarith))
        rw [ MeasureTheory.integrable_withDensity_iff_integrable_smul₀ ]
        · exact h_integrable
