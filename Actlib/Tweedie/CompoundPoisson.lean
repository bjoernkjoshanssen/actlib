...
-/
import Mathlib
import Actlib.Tweedie.Tweedie
import Actlib.Tweedie.GammaConvolution
import Actlib.Tweedie.CompoundPoissonCore
import Actlib.Tweedie.TweedieAux
...
lemma pi_sum_eq_zero {ι : Type*} [Fintype ι] [Nonempty ι] [NoAtoms μ] :
  ...
  -- By the measure-preserving property, the measure of the preimage of $t$ under $e$ is
  -- equal to the measure of $t$ under the product measure.
  have h_preimage : (Measure.pi (fun _ : ι => μ)) {g : ι → ℝ | ∑ i, g i = 0} =
    (Measure.prod (Measure.pi (fun _ : {i // p i} => μ))
      (Measure.pi (fun _ : {i // ¬p i} => μ))) t := by
    have h_preimage : (Measure.pi (fun _ : ι => μ)) {g : ι → ℝ | ∑ i, g i = 0} =
      (Measure.pi (fun _ : ι => μ)) (Set.preimage (MeasurableEquiv.piEquivPiSubtypeProd
        (fun _ : ι => ℝ) p) t) := by
      congr with g
      simp only [MeasurableEquiv.piEquivPiSubtypeProd_apply, Set.mem_setOf_eq, t]
      rw [show ( Finset.univ : Finset ι ) = Finset.image ( fun i : { i // p i } => i.val )
        Finset.univ ∪ { j } from ?_, Finset.sum_union] <;> norm_num [Finset.sum_image, p]
      · rw [show ( Finset.univ : Finset { i // ¬p i } ) = { ⟨j, by aesop⟩ } from
          Finset.eq_singleton_iff_unique_mem.mpr ⟨Finset.mem_univ _, fun x hx => by aesop⟩
        ]; simp [p]
  ...
  -- Since `{i // ¬ p i} = {i // i = j}` is a singleton type with unique element
  -- `j₀ := ⟨j, by simp⟩`, `∑ i, b i = b j₀`.
  -- Hence the slice is `{b | b j₀ = -(∑ i, a i)}`.
  have h_slice : ∀ a : ({i // p i} → ℝ), (Measure.pi (fun _ : {i // ¬p i} => μ))
    {b : {i // ¬p i} → ℝ | (∑ i, a i) + (∑ i, b i) = 0} = 0 := by
    intro a
    have h_singleton : {b : {i // ¬p i} → ℝ | (∑ i, a i) + (∑ i, b i) = 0}
      = {fun _ => -(∑ i, a i)} := by
      ext b
      simp only [ne_eq, Set.mem_setOf_eq, Set.mem_singleton_iff, p]
      rw [show ( Finset.univ : Finset { i // ¬p i } ) = { ⟨j, by aesop⟩ } from
        Finset.eq_singleton_iff_unique_mem.mpr ⟨Finset.mem_univ _,
        fun x hx => Subtype.ext <| by aesop⟩]
      simp only [Finset.sum_singleton, add_eq_zero_iff_eq_neg]
      exact ⟨fun h => funext fun x => by rw [
        show x = ⟨j, by aesop⟩ from Subtype.ext <| by aesop]; linarith, fun h => by rw [h]; simp⟩
    aesop
...
lemma infinitePi_sum_range_succ_eq_zero [NoAtoms μ] (m : ℕ) :
  ...
  -- By `MeasureTheory.Measure.infinitePi_map_restrict (fun _ : ℕ => μ)`, we have
  -- `Measure.map I.restrict (Measure.infinitePi (fun _ : ℕ => μ)) = Measure.pi (fun i : ↥I => μ)`.
  have h_map : Measure.map (fun f : ℕ → ℝ => fun i : I => f i) (Measure.infinitePi (fun _ : ℕ => μ))
    = Measure.pi (fun i : I => μ) := by
...
theorem compoundPoisson_singleton_zero [NoAtoms μ] :
  ...
  have h_integral : ∫⁻ n, (Measure.infinitePi (fun _ : ℕ => μ))
    {f : ℕ → ℝ | ∑ i ∈ Finset.range n, f i = 0} ∂(poissonMeasure lam)
    = (poissonMeasure lam) {0} := by
    ...
    · filter_upwards [] with n
      cases n <;> simp_all [infinitePi_sum_range_succ_eq_zero]
...
/-- Realization of the Tweedie distribution as a compound Poisson distribution
for `1 < p < 2`. -/
noncomputable def tweedieConstruction (μ : ℝ) (hμ : 0 ≤ μ) {φ p : ℝ}
    (hφ : 0 ≤ φ)
    (hp₂ : p < 2) :=
  compoundPoisson
...
/-- The gamma distribution with Tweedie parameters is a probability measure. -/
theorem tweedie_gamma_IsProbabilityMeasure {μ : ℝ} (hμ : 0 < μ) {φ p : ℝ}
    (hφ : 0 < φ) (hp₁ : 1 < p)
...
lemma tweedie_zero_sanity_check (μ : ℝ) (hμ : 0 < μ) {φ p : ℝ}
    (hφ : 0 < φ) (hp₁ : 1 < p)
    (hp₂ : p < 2) : tweedieConstruction μ (by linarith)
      (show 0 ≤ φ by linarith)
       hp₂ {0} = tweedieProbZero μ φ p := by
  unfold tweedieConstruction
...
lemma tweedie_zero_sanity_check₂ (μ : ℝ) (hμ : 0 < μ) {φ p : ℝ}
    ...
    let hμ₀ := le_of_lt hμ
    let hφ₀ := le_of_lt hφ
    tweedieConstruction μ hμ₀ hφ₀ hp₂ {0} =
...
/-- Closed form of a `twG` summand as a Poisson-weight times a gamma density, valid for `y > 0`.
...
lemma twG_eq_gamma (μ : ℝ) (hμ : 0 < μ) {φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hφ : 0 < φ)
    (j : ℕ) {y : ℝ} (hy : 0 < y) :
    twG μ φ p j y
      = Real.exp (-twZ μ φ p) * (twZ μ φ p) ^ j / (j.factorial)
        * gammaPDFReal ((j : ℝ) * ((2 - p) / (p - 1))) (μ ^ (1 - p) / (φ * (p - 1))) y := by
  have h1p : (1 - p) ≠ 0 := by linarith
  have hpm1 : (p - 1) ≠ 0 := by linarith
  rcases Nat.eq_zero_or_pos j with hj0 | hjpos
  · subst hj0
    rw [twG_zero, gammaPDFReal, if_pos hy.le]
...
open MeasureTheory ProbabilityTheory in
/-- For (Lebesgue-)almost every `y > 0`, the Tweedie term series `j ↦ twG μ φ p j y` is summable.
...
lemma twG_ae_summable (μ : ℝ) {φ p : ℝ} (hp₁ : 1 < p) (hp₂ : p < 2) (hμ : 0 < μ) (hφ : 0 < φ) :
    ∀ᵐ y ∂volume, y ∈ Set.Ioi (0 : ℝ) → Summable (fun j : ℕ => twG μ φ p j y) := by
...
lemma tweedie_density_series_ae (μ : ℝ) (hμ : 0 < μ) {φ p : ℝ} (hφ : 0 < φ) (hp₁ : 1 < p)
    ...
    (fun z => ∑' n : ℕ, ENNReal.ofReal
        (Real.exp (-twZ μ φ p) * (twZ μ φ p) ^ (n + 1) / ((n + 1).factorial))
  ...
  filter_upwards [twG_ae_summable μ hp₁ hp₂ hμ hφ, hzne] with z hzsum hzne0
  ...
  · -- z > 0 : use the pointwise Tweedie series
    have hpw : tweediePDF μ φ p z = ∑' j : ℕ, twG μ φ p j z := by
      simp only [tweediePDF]
      rw [Set.indicator_of_mem (by simp only [Set.mem_setOf_eq]; exact hzgt)]
      exact tw_pointwise μ φ p z
    rw [hpw,
      ENNReal.ofReal_tsum_of_nonneg
        (fun j => twG_nonneg μ φ p hp₁ hp₂ hφ j hzgt) (hzsum hzgt)]
    conv_rhs => rw [tsum_eq_zero_add' ENNReal.summable]
    rw [twG_zero, ENNReal.ofReal_zero, zero_add]
    have htwz : 0 ≤ twZ μ φ p := by rw [twZ]; positivity
    refine tsum_congr (fun n => ?_)
    rw [twG_eq_gamma μ hμ hp₁ hp₂ hφ (n + 1) hzgt,
      ENNReal.ofReal_mul
...
theorem tweedie_eq (μ : ℝ) (hμ : 0 < μ) {φ p : ℝ}
    ...
    let hμ₀ := le_of_lt hμ
    let hφ₀ := le_of_lt hφ
    tweedieConstruction μ hμ₀ hφ₀ hp₂ =
  ...
  · -- atom at 0
    rw [ENNReal.smul_def]
    congr 1
    rw [tweedieProbZero, ENNReal.ofReal_eq_coe_nnreal (Real.exp_nonneg _)]
    congr 1
    apply NNReal.coe_injective
    change Real.exp (-(μ ^ (2 - p) / (φ * (2 - p))))
