...
For `1 < p < 2` the Tweedie exponential dispersion model with mean `μ` and dispersion `φ`
has a density supported on `(0, ∞)` whose integral is `1 - exp(-μ^(2-p) / (φ (2-p)))`; the
