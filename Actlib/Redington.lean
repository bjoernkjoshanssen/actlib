...
lemma second_D_mul {f g : ℝ → ℝ} (df : Differentiable ℝ f) (d₂f : Differentiable ℝ (deriv f))
    (dg : Differentiable ℝ g) (d₂g : Differentiable ℝ (deriv g)) :
...
lemma exp_D {f : ℝ → ℝ} (df : Differentiable ℝ f) : deriv (fun x : ℝ => exp (f x)) =
  fun x : ℝ => exp (f x) * deriv f x := funext (fun x => deriv_exp (df x))

lemma const_mul_D (a : ℝ) : deriv (fun x : ℝ => a * x) = fun _ : ℝ => a := by
  ext y
  rw [deriv_const_mul]
...
lemma hf2_justification {f g₀ : ℝ → ℝ} {δ : ℝ}
  ...
  have h₂ : deriv f δ * g₀ δ + f δ * deriv g₀ δ = deriv g δ := by exact
    (congrFun h₁ δ).symm
  have h₃ : deriv f δ * g₀ δ = 0 := by
    rw [hf] at h₂; simp only [zero_mul, add_zero] at h₂; rw [hg] at h₂; rw [h₂]
...
lemma d₂g_form_simplified_justification {f g₀ : ℝ → ℝ} {δ : ℝ}
    ...
    (dg₀ : Differentiable ℝ g₀) (d₂g₀ : Differentiable ℝ (deriv g₀))
    (hf : f δ = 0) (hf' : (deriv f) δ = 0) :
    (deriv (deriv (f * g₀))) δ = (deriv (deriv f)) δ * g₀ δ :=
  show (deriv (deriv (f * g₀))) δ = (deriv (deriv f)) δ * g₀ δ by
  calc
    _ = (deriv (deriv f) * g₀ + 2 * deriv f * deriv g₀ + f * deriv (deriv g₀)) δ :=
      congrFun (second_D_mul df d₂f dg₀ d₂g₀) δ
    _ = deriv (deriv f) δ * g₀ δ + 2 * deriv f δ * deriv g₀ δ + f δ * deriv (deriv g₀) δ  := by rfl
    _ = deriv (deriv f) δ * g₀ δ + 2 * deriv f δ * deriv g₀ δ + 0 * deriv (deriv g₀) δ  := by
      rw [hf]
...
theorem g_form_implies_redington_f_iff_g {f : ℝ → ℝ} {t δ : ℝ}
  ...
  let g := fun x : ℝ => (f x) * exp (t * x)
  (redington f δ ↔ redington g δ) :=
  let g := fun x : ℝ => (f x) * exp (t * x)
  let g₀_arg   := fun x : ℝ => t * x
  let g₀       := fun x : ℝ=> exp (t * x)
  let f'       := deriv f
  let f''      := deriv f'
  let g'       := deriv g
  let g''      := deriv g'
  ...
  have dg₀_arg_form : deriv g₀_arg = fun _ : ℝ => t := by rw [const_mul_D t]
  have dg₀_form_lemma : ∀ y : ℝ, (fun (x : ℝ) => exp (g₀_arg x) * (fun (_ : ℝ) => t) x) y
    = (fun (x : ℝ) => t * exp (t * x)) y := by
    intro y
    exact mul_comm (exp (g₀_arg y)) ((fun _ => t) y)
  have dg₀_form : deriv g₀ = fun x : ℝ => t * exp (t * x) := by
    rw [exp_D dg₀_arg]; rw [dg₀_arg_form]; exact funext dg₀_form_lemma
  have d₂g₀ : Differentiable ℝ (deriv g₀) := by
    rw [dg₀_form]; apply Differentiable.mul (differentiable_const t) (dg₀)
