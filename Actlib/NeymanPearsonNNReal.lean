...
We use `ℝ≥0` instead of `ℝ≥0∞` which may prevent Radon-Nikodym
generalization. However, we follow the Wikipedia
argument which does not seem to work well with `R≥0∞`.
 -/

noncomputable section

/-- The Neyman-Pearson region. -/
def RNP (θ₀ θ₁ η : ℝ) (ρ : ℝ → ℝ → NNReal) : Set ℝ :=
    { x | ρ θ₁ x - η * ρ θ₀ x ≥ 0}

theorem NP.intRNP₀ {θ₀ : ℝ} {R : Set ℝ} {ρ : ℝ → ℝ → NNReal}
