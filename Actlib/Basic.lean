...
Although the PV of 1 varies continuously,
only PV of 1 at integer times contribute to the PV of the annuity.
-/
noncomputable def a : ℕ → ℝ := fun n => Finset.sum (Finset.univ : Finset (Fin n))
    fun t => (1:ℝ) / acc (t.1+1)

/-- Present value of deferred (by `m` periods) annuity-immediate
(with `n` installments), general case. -/
...
theorem present_value_continuous_annuity_simple_interest_le_future_value.extracted
  {r n x : ℝ} (hr : 0 ≤ r) (hn : 0 ≤ n) (hx : x ∈ Set.Ioo 0 n) :
      1 / (1 + r * x) ≤ 1 + r * (n - x) := by
    simp at hx ⊢
    field_simp
    have hr₀ : 0 < 1 + r * x := by
  ...
  · apply intervalIntegral.intervalIntegrable_one_div
    · intro x hx
      have : 0 ≤ x := by
        have h₁ := hx.1
        have h₂ := hx.2
        simp only [inf_le_iff, le_sup_iff] at h₁ h₂
        cases h₁ with
        | inl h => tauto
        | inr h =>
            cases h₂ with
    ...
    exact continuous_const_mul r
  · apply IntervalIntegrable.add
    · simp
    · apply IntervalIntegrable.const_mul <| IntervalIntegrable.sub (by simp) (by simp)
  intro
  exact present_value_continuous_annuity_simple_interest_le_future_value.extracted hr hn
...
lemma annuity_positive {n : ℕ} (hn : n ≠ 0) {i : ℝ} (hi : i > -1) :
  ...
  suffices ∑ k ∈ Icc 1 n, (1 + i)⁻¹ ^ k >
     ∑ k ∈ Icc 1 n, 0 by simp only [inv_pow, sum_const_zero, gt_iff_lt] at this ⊢;exact this
...
lemma annuity_nonnegative (n : ℕ) {i : ℝ} (hi : i > -1) :
  ...
  suffices  ∑ k ∈ Icc 1 n, (1 + i)⁻¹ ^ k ≥
     ∑ k ∈ Icc 1 n, 0 by
      simp only [inv_pow, sum_const_zero, ge_iff_le] at this ⊢
      exact this
  refine sum_le_sum ?_
  intro k hk
  apply le_of_lt
...
theorem annuity_value_bounded {i : ℝ} (hi : i > 0) (n : ℕ) :
    (a_formula n ⌝ i) ≤ 1 / i :=
  div_le_div₀ zero_le_one (by
    ring_nf
    simp only [inv_pow, tsub_le_iff_right, le_add_iff_nonneg_right, inv_nonneg]
    positivity) hi (by simp)
...
theorem annuity_with_interest_one {n : ℕ} : (a_formula n ⌝ 1) = 1 - (1/2) ^ n := by
    simp only [a_formula, inv_pow, div_one, one_div, sub_right_inj, inv_inj]
    congr
    exact one_add_one_eq_two

theorem annuity_with_interest_half {n : ℕ} : (a_formula n ⌝ (1/2)) = 2 * (1 - (2/3) ^ n) := by
    simp only [a_formula, one_div, inv_pow, div_inv_eq_mul]
...
lemma N_eq_CPT_N {IY PMT PV FV : ℝ} {N : ℕ} (h : annuity_equation IY PMT PV FV N)
  (h₀ : IY ≠ 0) (h₁ : IY ≠ -100) (h₂ : FV * (IY / 100) - PMT ≠ 0)
  (h₄ : IY / 100 ≠ -2) :
  ...
  have g₀ : Real.log (1 + IY / 100)⁻¹ ≠ 0 := by
    simp only [Real.log_inv, ne_eq, neg_eq_zero, Real.log_eq_zero, add_eq_left, div_eq_zero_iff,
      OfNat.ofNat_ne_zero, or_false, not_or]
    constructor
    · contrapose! h₁
      linarith
    constructor
...
lemma annuity_equation_continuity {PMT PV FV i : ℝ} {N : ℕ} :
    ContinuousOn (fun i ↦ PV + PMT * annuity.a N i + FV * (1 + i)⁻¹ ^ N) (Set.Icc 0 i) := by
        ...
        · refine Continuous.comp_continuousOn' ?_ ?_
          · exact continuous_const_add PV
          refine Continuous.comp_continuousOn' ?_ ?_
          · exact continuous_const_mul PMT
          unfold annuity.a
          refine Continuous.comp_continuousOn' ?_ ?_
          · exact continuous_finsetSum _ fun i a ↦ continuous_apply i
          refine continuousOn_pi.mpr ?_
          intro k
          refine ContinuousOn.pow ?_ k
          refine ContinuousOn.inv₀ ?_ ?_
          · apply Continuous.continuousOn <| continuous_const_add 1
          intro x hx
          simp at hx
          apply ne_of_gt
          linarith
        refine Continuous.comp_continuousOn' ?_ ?_
        · exact continuous_const_mul FV
...
lemma annuity_antitone {N : ℕ} (hN : N ≠ 0) ⦃a b : ℝ⦄ (hab : a < b) (ha : 0 ≤ a) :
  annuity.a N b < annuity.a N a := by
        ...
        apply sum_lt_sum
        · intro t ht
          simp only [mem_Icc, inv_pow] at ht ⊢
          refine inv_anti₀ ?_ ?_
          · positivity
...
theorem CPT_IY_unique {PMT PV FV : ℝ} {N : ℕ} (hN : N ≠ 0)
  ...
  have ⟨i,hi⟩ : ∃ i ≥ 0, f i = 0 := by
    ...
    have h₀ : ι > 0 ∧ f ι < 0 := by
        ...
        · unfold f
          ...
          have := this ι hι
          apply lt_trans
          · change PV + PMT * annuity.a N ι + FV * (1 + ι)⁻¹ ^ N <
                 PV + PMT * (1 / ι) + FV * (1 + ι)⁻¹ ^ N
            have : PMT * annuity.a N ι <
              PMT * (1 / ι) := by
                (expose_names; exact (mul_lt_mul_iff_of_pos_left hPMT).mpr (this_1 ι hι))
            linarith
          calc _ ≤ PV + PMT * (1 / ι) + FV * (1 + ι)⁻¹ := by
                suffices PMT * (1 / ι) + FV * (1 + ι)⁻¹ ^ N
                  ≤ PMT * (1 / ι) + FV * (1 + ι)⁻¹ by linarith
               ...
               _ < _ := by
                unfold ι
                have : PMT * (1 / (2 * max FV PMT / -PV))
                     = PMT * (-PV / (2 * max FV PMT)) := by field_simp
                rw [this]
                ...
                have : 1 / (1 + 2 * max FV PMT / -PV)
                  = ((-PV) * 1) / (-PV * (1 + 2 * max FV PMT / -PV)) := by
                    ...
                    linarith
                rw [this]
                have : -PV * (1 + 2 * max FV PMT / -PV)
                   = ((-PV) * 1 + (-PV) * (2 * max FV PMT / -PV)) := by ring_nf
                rw [this]
                ...
                have :  PV + PMT * (-PV / (2 * max FV PMT))
                  + FV * (-PV * 1 / (-PV * 1 + 2 * max FV PMT))
                     =  PV * (1 + PMT * (-1 / (2 * max FV PMT))
                      + FV * (-1 * 1 / (-PV * 1 + 2 * max FV PMT))) := by ring_nf
                rw [this]
                apply mul_neg_of_neg_of_pos hPV
                simp only [mul_one]
                suffices - ( PMT * (-1 / (2 * max FV PMT)) + FV
                  * (-1 / (-PV + 2 * max FV PMT))) < 1 by linarith
    ...
    have :  0 ∈ Set.Icc (f i) (f 0) := by
        ...
        simp
        tauto
    have ⟨j,hj⟩:= @intermediate_value_Icc' ℝ _ _ _ _ ℝ _ _ _ 0 i
  ...
  have ha: StrictAntiOn f (Set.Ici 0) := by
    unfold f
    intro a ha b hb hab
    simp only [Set.mem_Ici, inv_pow] at ha hb ⊢
    suffices PMT * annuity.a N b + FV * ((1 + b) ^ N)⁻¹
      < PMT * annuity.a N a + FV * ((1 + a) ^ N)⁻¹ by
        linarith
    have : ((1 + b) ^ N)⁻¹ < ((1 + a) ^ N)⁻¹ := by
    ...
    have : PMT * annuity.a N b  < PMT * annuity.a N a := by
      have : annuity.a N b < annuity.a N a := by
        apply annuity_antitone hN hab ha
      exact (mul_lt_mul_iff_of_pos_left hPMT).mpr this
    have : FV * ((1 + b) ^ N)⁻¹ <  FV * ((1 + a) ^ N)⁻¹ := by
      have : ((1 + b) ^ N)⁻¹ < ((1 + a) ^ N)⁻¹ := by
        tauto
      (expose_names; exact (mul_lt_mul_iff_of_pos_left hFV).mpr this_1)
    linarith
  have : ∃! i ≥ 0, f i = 0 := by
    use i
    constructor
  ...
  · ring_nf
    intro J hJ
    have := hi.2 (J/100)
    simp only [ge_iff_le, one_div, inv_pow, and_imp] at hJ ⊢ this
...
lemma PMT_eq_CPT_PMT {IY PMT PV FV : ℝ} {N : ℕ} (h : annuity_equation IY PMT PV FV N)
  ...
  have : annuity.a N (IY / 100) ≠ 0 := by
    ...
    rw [congrFun this]
    unfold annuity.a_formula
    simp only [inv_pow, ne_eq, div_eq_zero_iff, OfNat.ofNat_ne_zero, or_false, not_or]
    constructor
    · field_simp
      contrapose! h₁
      have This : 100 ^ N / (100 + IY) ^ N
        = (100 / (100 + IY)) ^ N := by ring_nf
      rw [div_pow] at h₁
      simp only [one_div, inv_div] at h₁
      rw [This] at h₁
      have : ((100) / ((100) + IY)) ^ N = 1 ^ N := by
        field_simp
        simp
        linarith
      have : (100) / ((100) + IY) = 1 := by
        ...
        simp only [mul_eq_zero, Nat.cast_eq_zero, Real.log_eq_zero] at this
        cases this with
        | inl h => omega
        | inr h =>
          cases h with
