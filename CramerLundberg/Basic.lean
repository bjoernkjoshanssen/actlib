...
-- Time it takes to reach 2 if we interpret (0,1) as (-1,1)

open Filter Order TopologicalSpace WithTop Filtration

noncomputable def Filtration.piLE : Filtration ℕ (inferInstance : MeasurableSpace (ℕ → ℕ)) where
  seq n := ⨆ i ≤ n, MeasurableSpace.comap (fun ω : ℕ → ℕ => ω i) ⊤
  mono' i j hij := by
    apply iSup_le; intro k; apply iSup_le; intro hk
    exact le_iSup₂ (f := fun i (_ : i ≤ j) => MeasurableSpace.comap (fun ω : ℕ → ℕ => ω i) ⊤) k (le_trans hk hij)
  le' n := by
    apply iSup_le; intro i; apply iSup_le; intro _
    exact le_iSup (fun i => MeasurableSpace.comap (fun ω : ℕ → ℕ => ω i) ⊤) i
...
-- example : IsStoppingTime Filtration.piLE T := by
--     let A (i : WithTop ℕ) (ω : ℕ → ℕ) : Prop :=
--             T ω ≤ i
--     intro i
--     by_cases h₀ : i = 0
--     subst i
--     simp
--     have : fun ω => T ω = 0 = fun ω => False := by sorry
--     rw [this]
--     simp
--     by_cases h₁ : i = 1
--     subst i
--     simp
--     have : fun ω => T ω ≤ 1 = fun ω => False := by sorry
--     rw [this]
--     simp
--     by_cases h₂ : i = 2
--     subst i
--     simp
--     have : fun ω => T ω ≤ 2 = fun ω => ω 0 = 1 ∧ ω 1 = 1 := by sorry
--     rw [this]
...
--     fin_cases j
--     all_goals simp
--     · have : A 0 = fun ω => False  := by
--         ext ω;unfold A T
--         split_ifs <;>
--         · simp only [Fin.isValue, iff_false, not_le]
--           exact sign_eq_one_iff.mp rfl
--       unfold A T at this
--       unfold T
--       simp_rw [this]
--       simp
--     · have : A 1 = fun ω => False  := by
--         ext ω; unfold A T
--         split_ifs <;>
--         · simp only [Fin.isValue, iff_false, not_le]
...
--       simp
--     · unfold T
--       have : A (WithTop.some 2) = fun ω => ω 0 = 1 ∧ ω 1 = 1 := by
--         unfold A T
--         ext ω
--         split_ifs <;> tauto
--       unfold A T at this
--       simp_rw [this]
--       apply?
--       unfold Filtration.piLE
--       refine Measurable.eq_const ?_ 2
--       refine Measurable.add ?_ ?_
--       sorry
--       sorry
--     · have : A (WithTop.some 3) = fun ω => (ω 0) + (ω 1) = 2 := by
--         unfold A T
--         ext ω
--         split_ifs <;> tauto
--       unfold A T at this
...
--       simp_rw [this]
--       sorry
--     · have : A (WithTop.some 4) = fun ω =>
--         (ω 0).1 + (ω 1).1 = 2
--         ∨
--         (ω 0 = 0 ∧ ω 1 = 1 ∧ ω 2 = 1 ∧ ω 3 = 1)
--         ∨
--         (ω 0 = 1 ∧ ω 1 = 0 ∧ ω 2 = 1 ∧ ω 3 = 1)
--          := by
--         unfold A T
--         ext ω
--         split_ifs <;> tauto
